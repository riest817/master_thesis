...
mutationToDomは，ブロックがXMLに書き込まれるたびに呼び出される．このAPIが存在しないかnullを返す場合，mutationは記録されない．APIが存在し，`mutation' XML 要素を返す場合，この要素（およびプロパティまたは子要素）は，ブロックのXML表現の先頭に保存される．
...
Blockly for Haskell で関数定義ブロックを実装した．図\ref{fig:haskell_function_new}に，関数定義ブロックを示す．関数定義ブロックでは，Mutator機能で関数の仮引数の数を自由に設定することができ，Mutatorダイアログのなかの引数ブロックを結合コンテナブロックに組み立てることで仮引数を調整できる．「=」の左部分のソケットに引数ブロックを「=」右部分に式ブロックを挿入できるようにした．
